...
import { ArchiveIcon, CheckCircleIcon, EyeIcon, MoreHorizontalIcon, SearchIcon, TrashIcon } from 'lucide-react';
...
export default function ContactsPage() {
  ...
  const handleBulkUpdateStatus = (status: ContactStatus) => {
    setContacts(contacts.map((contact) => (selectedContacts.includes(contact.id) ? { ...contact, status } : contact)));
    setSelectedContacts([]);
    ...
    <div className="space-y-6">
          ...
          <TableHeader>
            <TableRow>
              <TableHead className="w-12">
                <Checkbox checked={filteredContacts.length > 0 && selectedContacts.length === filteredContacts.length} onCheckedChange={handleSelectAll} />
              </TableHead>
            ...
            {filteredContacts.length > 0 ? (
              filteredContacts.map((contact) => (
                <TableRow key={contact.id}>
                  <TableCell>
                    <Checkbox checked={selectedContacts.includes(contact.id)} onCheckedChange={() => handleSelectContact(contact.id)} />
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell>
                    <div className="text-sm">
                      <div>{contact.type || 'Not specified'}</div>
                      <div className="text-xs text-muted-foreground">{contact.budget || 'No budget specified'}</div>
                    </div>
                  </TableCell>
                  <TableCell>
                    <div className="max-w-xs truncate">{truncateText(contact.message)}</div>
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell>
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        ...
                        <DropdownMenuItem onClick={() => handleUpdateStatus(contact.id, 'IN_PROGRESS')}>
                          <CheckCircleIcon className="mr-2 h-4 w-4" /> Mark In Progress
                        </DropdownMenuItem>
                        <DropdownMenuItem onClick={() => handleUpdateStatus(contact.id, 'COMPLETED')}>
                          <CheckCircleIcon className="mr-2 h-4 w-4" /> Mark Completed
                        </DropdownMenuItem>
                        <DropdownMenuItem onClick={() => handleUpdateStatus(contact.id, 'ARCHIVED')}>
                          <ArchiveIcon className="mr-2 h-4 w-4" /> Archive
                        </DropdownMenuItem>
                        <DropdownMenuItem onClick={() => handleDeleteContact(contact.id)} className="text-destructive focus:text-destructive">
      ...
      {selectedContacts.length > 0 && (
        <div className="fixed bottom-4 left-1/2 -translate-x-1/2 rounded-lg border bg-card p-4 shadow-lg">
          <div className="flex items-center gap-4">
            <span className="text-sm">{selectedContacts.length} contact(s) selected</span>
            <Button variant="outline" size="sm" onClick={() => handleBulkUpdateStatus('IN_PROGRESS')}>
              Mark In Progress
            </Button>
